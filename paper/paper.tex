...
This paper presents FMIExchange.jl \cite{bex2025e}, which provides utilities to simplify interfacing FMUs with the popular DifferentialEquations.jl package \cite{rackauckas2017}.
...
\begin{figure}
  \center
\begin{tikzpicture}[node distance=0.5cm, >={Stealth[round]}, scale=1.0, transform shape]
...
\node (ME) [below=0.75cm of FMUeq, draw=black, label={[name=MEL]below:\footnotesize Model Exchange}] {External Solver};
...
\draw [<-] (e2) -- node [auto, yshift=-1mm] {$u$} node [auto, swap, yshift=-1mm] {\footnotesize \shortstack{event triggers}} ($(ME.north west)!(e2)!(ME.north east)$);
...
For the addressing and state/input/output bundling functionality, FMIExchange.jl relies on custom types \typename{AbstractSimModel} and \typename{AbstractModelSpecification}.
Addresses in the form of indices of \texttt{p}/\texttt{u} and model functionality (dynamics function and callbacks) are contained in the \typename{AbstractSimModel}.
The \typename{AbstractSimModel} (or a vector of them) is used when bundling the dynamics functions and generating callbacks for automatically calling model output functions between integration steps.
To avoid having to manually assign addresses and provide a human-friendly interface, FMIExchange.jl can automatically generate address maps from human-readable \typename{AbstractModelSpecification}s.
\typename{AbstractModelSpecification}s specify states/inputs/outputs by name rather than address and can be converted directly into \typename{AbstractSimModel}s using an address map.

The demo \cite{bex2024} provides a practical example of an FMIExchange.jl worklflow.
...
The effect of this model mismatch is studied through dynamic simulation of the system.
...
Note that it is possible to simulate multiple FMUs and native ODEs using solely FMI.jl and DifferentialEquations.jl, but FMIExchange.jl greatly simplifies this process through its \typename{AbstractSimModel} interface.
