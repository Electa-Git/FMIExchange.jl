...
Compliant DAE models (so-called Functional Mock-up Units (FMUs)) can be simulated through a standardized interface, which allows to easily exchange these models between different software tools.
...
This paper presents FMIExchange.jl, which complements existing FMI packages by interfacing FMUs with the popular DifferentialEquations.jl package \cite{rackauckas2017}.
Additionally, FMIExchange.jl provides tools to compose simulations containing multiple FMUs and native Julia models.

\end{abstract}

\section{Statement of Need}
An FMU (\cref{fig:FMU}) transforms a DAE with events\footnote{Events here signify discontinuities in the solution: time-based events are discontinuities at fixed times, typically implemented through discrete callbacks in DifferentialEquations.jl; state-based events depend on the integrated quantities and are typically implemented with continuous callbacks.} \cref{eq:DAE} into an ordinary differential equation (ODE) with events \cref{eq:ODE}.

In addition to the states $u$ and inputs $p_i$ required for simulating the model, the FMU can define arbitrary outputs $p_o$ \cref{eq:output} that are exposed to the user.
Two types of FMU are available: Co-Simulation FMUs only expose inputs and outputs and come bundled with an integrator and event handler, Model Exchange (ME-)FMUs require an external ODE integrator/event handler and expose all quantities; this text focuses on ME-FMUs.
\begin{align}
F(\dot{u}, u, p_{i}, t) &= 0 && \mathrm{events}\label{eq:DAE}\\
f(u, p_{i}, t) &= \dot{u} && \mathrm{events}\label{eq:ODE}\\
...
\begin{tikzpicture}[node distance=0.5cm, >={Stealth[round]}]
  \node (FMU) [draw=black, label=above:FMU, minimum width=5cm] {$\begin{aligned}
    0 &= F(\dot{u}, u, p_i, t)\\
    p_o &= g(u, p_i, t)
  \end{aligned}$};
...
\node (OR) [below=1.5cm of FMU] {OR};
\node (CS) [right=of OR, draw=black, label={[name=CSL]below:\footnotesize Co-Simulation}] {\shortstack{Internal\\Solver}};

\node (ME) [left=of OR, draw=black, label={[name=MEL]below:\footnotesize\bfseries Model Exchange}] {\shortstack{\bfseries External\\ \bfseries Solver}};
\node (solver) [fit=(MEL) (ME) (CSL) (CS) (OR), draw=black] {};
\draw[->] (FMU.south -| CS) -| node [near end, auto] {$\dot{u}$} node [near end, auto, swap] {\footnotesize \shortstack{event\\indicators}} (solver.north -| CS);
\draw[->] (solver.north -| ME) -| node [near end, auto, swap] {$u$} node [near end, auto] {\footnotesize \shortstack{event\\triggers}} (FMU.south -| ME);
\end{tikzpicture}
\caption{FMUs present a standardized interface for describing DAEs}
\label{fig:FMU}
\end{figure}

An FMU bundles a DAE model into a (binary) shared library and accompanying XML description of the model; this allows to call the model from other programs e.g. using Julia's \texttt{ccall} functionality.
...
FMIExchange.jl does not provide any simulation capabilities itself; it merely generates callbacks/dynamics functions\ldots to use in the DifferentialEquations.jl framework.
...
Addresses in the form of indices of \texttt{p}/\texttt{u} and model functionality (dynamics function\ldots) are contained in the \texttt{AbstractSimModel}.
